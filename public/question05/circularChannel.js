class circularChannel {
  ...
  drawTabletop(sizeX = 400, sizeY = 400, colorVal = 230) {
    push();
    translate(this.center.x, this.center.y, this.center.z - 0.5); // slightly below Z=0
    // rotateX(HALF_PI); // XY plane
    fill(colorVal, colorVal, colorVal, 250); // R, G, B, A (alpha = 100 out of 255)
  ...
  drawLabeledPoints(labelSize = 24) {
    ...
    // P (at phi0)
    this._drawLabeledPoint(this.phi0, "P", -12, -12);
  
    // Q (midpoint)
    const phiQ = (this.phi0 + this.phi1) / 2;
    this._drawLabeledPoint(phiQ, "Q", -12, 12);
  
    // R (at phi1)
    this._drawLabeledPoint(this.phi1, "R", 14, 0);
  }
